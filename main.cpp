...
HRESULT APIENTRY DrawIndexedPrimitive_hook(IDirect3DDevice9* pDevice, D3DPRIMITIVETYPE Type, INT BaseVertexIndex, UINT MinVertexIndex, UINT NumVertices, UINT startIndex, UINT primCount)
{
	...
	//aim at team 1
	if(aimbot==1|| textesp==1 || circleesp==1 || lineesp==1)
	//if ((Stride == 32) && (NumVertices == countnum))
	if ((Stride == 32 && numElements == 7 && decl->Type == 2) && (NumVertices == 36||NumVertices == 38))//24hand,64teeth,86eyes, 38,36teamA, 30teamB,98,86,32
	{
		AddModels(pDevice, 1);
	}

	//aim at team 2
	if (aimbot == 2 || textesp==2 || circleesp==2 || lineesp==2)
	//if ((Stride == 32) && (NumVertices == countnum))
	if ((Stride == 32 && numElements == 7 && decl->Type == 2) && (NumVertices == 30 || NumVertices == 98 || NumVertices == 86 || NumVertices == 32))//24hand,64teeth,86eyes, 38,36teamA, 30teamB,98,86,32
	{
		AddModels(pDevice, 2);
	}

	if (chams)
	if (Stride == 32 && numElements == 7 && decl->Type == 2)// && (NumVertices == 36 || NumVertices == 38 || NumVertices == 30 || NumVertices == 98 || NumVertices == 86 || NumVertices == 32))
	{
		pDevice->SetTexture(3, Red);
		pDevice->SetRenderState(D3DRS_ZENABLE, FALSE);
		DrawIndexedPrimitive_orig(pDevice, Type, BaseVertexIndex, MinVertexIndex, NumVertices, startIndex, primCount);
		pDevice->SetRenderState(D3DRS_ZENABLE, TRUE);
		pDevice->SetTexture(3, Green);
		/*
		float bias = 1000.0f;
...
HRESULT APIENTRY Present_hook(IDirect3DDevice9* pDevice, const RECT *pSourceRect, const RECT *pDestRect, HWND hDestWindowOverride, const RGNDATA *pDirtyRegion)
{
	...
	if (!is_imgui_initialised)
	{
		...
		//old generate texture
		//D3DXCreateTextureFromFileInMemory(pDevice, (LPCVOID)&bRed, sizeof(bRed), &texRed);
		//D3DXCreateTextureFromFileInMemory(pDevice, (LPCVOID)&bGreen, sizeof(bGreen), &texGreen);
	...
	if (info)
	{
		ImGui::SetNextWindowSize(ImVec2(260.0f, 20.0f)); //size
		...
		ImGui::Text("Hack Loaded, press INSERT for menu");
		ImGui::End();
	...
	if (ShowMenu)
	{
		...
		//ImGui::SliderInt("Aimbot", &aimbot, 1, 2);
		ImGui::Text("Aimbot");
		ImGui::SameLine();
		const char* aimbot_Options[] = { "Off", "EU", "NRF" };
		ImGui::Combo("##Aimbot", (int*)&aimbot, aimbot_Options, IM_ARRAYSIZE(aimbot_Options));

		//ImGui::Checkbox("Distance Esp", &textesp);
		ImGui::Text("Esp");
		ImGui::SameLine();
		const char* textesp_Options[] = { "Off", "EU", "NRF" };
		ImGui::Combo("##textesp", (int*)&textesp, textesp_Options, IM_ARRAYSIZE(textesp_Options));

		//ImGui::Checkbox("Circle Esp", &circleesp);
		ImGui::Text("Circle Esp");
		ImGui::SameLine();
		const char* circleesp_Options[] = { "Off", "EU", "NRF" };
		ImGui::Combo("##CircleEsp", (int*)&circleesp, circleesp_Options, IM_ARRAYSIZE(circleesp_Options));

		ImGui::Text("Line Esp");
		ImGui::SameLine();
		//const char* lineesp_Options[] = { "Up", "Center", "Down", "Off" };
		const char* lineesp_Options[] = { "Off", "EU", "NRF" };
		...
		ImGui::SliderInt("countnum", &countnum, -1, 250); //debug
		ImGui::Text("");
		ImGui::Text("Use MOUSE or TAB + ARROWS to navigate");
		ImGui::Text("SPACE to enable/disable features");
		ImGui::End();
	}
	...
	//do esp
	//2 = EU, Off = NRF
	if (textesp>0 || circleesp>0 || lineesp > 0)
	{
		...
		if (ModelEspInfo.size() != NULL)
		{
			for (unsigned int i = 0; i < ModelEspInfo.size(); i++)
			{
				...
				{
					//draw cricle
					if (circleesp == ModelEspInfo[i].iTeam)
						ImGui::GetWindowDrawList()->AddCircle(ImVec2(ModelEspInfo[i].pOutX, ModelEspInfo[i].pOutY), 2000 / ModelEspInfo[i].RealDistance, IM_COL32(255, 255, 255, 255), 12, 2.0f); //scale with distance

					//draw line
					if (lineesp == ModelEspInfo[i].iTeam)
					...
					//draw text
					if (textesp == ModelEspInfo[i].iTeam)
					{
						ImGui::GetWindowDrawList()->AddText(ImGui::GetFont(), ImGui::GetFontSize(), ImVec2(ModelEspInfo[i].pOutX, ModelEspInfo[i].pOutY), ImColor(255, 255, 0, 255), "Enemy", 0, 0.0f, 0); //draw text
						//if (ModelEspInfo[i].RealDistance > 0.0f && ModelEspInfo[i].RealDistance < 200.0f)
	...
	if (aimbot>0)
		if (ModelEspInfo.size() != NULL)
		{
			...
			for (unsigned int i = 0; i < ModelEspInfo.size(); i++)
			{
				...
				//aim at team 1 or 2
				if (aimbot == ModelEspInfo[i].iTeam)
...
{
	/*
	//Anti-occlusion v2 (used by chams to see models through walls at all distances, reduces fps, on/off requires restart)
...
DWORD WINAPI CDimgui(LPVOID lpParameter)
{
	...
	DetourAttach(&(LPVOID&)CreateQuery_orig, (PBYTE)CreateQuery_hook);
